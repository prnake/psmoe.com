...
因为对 flag 的赋值操作自身就可能造成**竞争条件**，导致非预期结果。
...
其中`sem_t`可视作`unsigned int`，`sem_post()`为信号量增加 1，`sem_wait(&sem)`持续检测信号量，直到信号量大于 0 才继续运行，并将信号量减 1。 
...
下面给出 c 语言的线程安全函数和非线程安全函数。（来自 [Thread-safe C library functions](http://www.keil.com/support/man/docs/armlib/armlib_chr1359122864458.htm) 和 [C library functions that are not thread-safe](http://www.keil.com/support/man/docs/armlib/armlib_chr1359122864707.htm)）
